pipeline {
    ...
    stages {
        stage('Init') {
            ...
            steps {
                script {
                    def dependenciesStr = "org.adempiere.ui, org.krysalis.barcode4j, org.idempiere.zk.extra, wrapped.org.jfree.jfreechart, wrapped.org.jfree.jcommon, slf4j-api"
                    ...
                    <b>Plugin:</b> ${env.PLUGIN_NAME}<br/>
                    <b>iDempiere:</b> ${env.IDEMPIERE_VERSION}<br/>
                    <b>Repositorio:</b> GitHub<br/>
                    <b>Ultima rama ejecutada:</b> ${env.BRANCH_NAME ?: 'N/A'}<br/>
                    ...
                    echo "Build description updated. Job descriptions are updated only on master."
                }
            }
        }
        stage('Compile') {
            ...
            steps {
                script {
                    if (fileExists('README.md')) {
                        writeFile file: 'generate_readme.py', text: '''import sys, re
...
<head>
    ...
    <style>
        ...
        table tr {{ background-color: var(--table-odd-bg); }}
        table tr:nth-child(2n) {{ background-color: var(--table-even-bg); }}
    </style>
</head>
<body>
